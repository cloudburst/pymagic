...
def guess_file(filepath, flags=MAGIC_NONE):
    mc=magic_open(flags)
    magic_load(mc, None)
...
def guess_string(string, flags=MAGIC_NONE):
    mc=magic_open(flags)
    magic_load(mc, None)

    res = magic_buffer(mc, string, len(string))

    magic_close(mc)
    return res

def find_in_file(filepath, flags=MAGIC_NONE, step=1024):
    mc=magic_open(flags)
    magic_load(mc, None)
    ...
    if os.path.exists(filepath):    
        with open(filepath, 'rb') as f:
            data = f.read()
        for chunk in xrange(step, len(data), step):
            res = magic_buffer(mc, data[chunk-step:chunk], step)
            if res != 'data':
                print "0x%x: %s" % (chunk-step, res)
    else:
        magic_close(mc)
        print "The file does not exist"
        return
